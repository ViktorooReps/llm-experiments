...
def get_slimpajama_large(num_proc: int = 40, num_rows: int = 100_000):
    if not os.path.exists(os.path.join(SPJ_DATA_PATH_LARGE, "train.bin")):
        os.makedirs(SPJ_DATA_PATH, exist_ok=True)
        dataset = load_dataset("cerebras/SlimPajama-627B", split=f"train[0:{num_rows}]")
